...
(function ($) {
        ...
        $.bsCalendar = {
            ...
            DEFAULTS: {
                ...
                icons: {
                    day: 'fa fa-calendar-days',
                    week: 'fa fa-calendar-days',
                    month: 'fa fa-calendar',
                    year: 'fa fa-calendar-days',
                    add: 'fa fa-plus',
                    menu: 'fa fa-bars',
                    search: 'fa fa-search',
                    prev: 'fa fa-chevron-left',
                    next: 'fa fa-chevron-right',
                    link: 'fa fa-arrow-up-right-from-square',
                    appointment: 'fa fa-clock',
                    appointmentAllDay: 'fa fa-sun'
                },
                url: null,
                queryParams: null,
                topbarAddons: null,
        ...
        function buildLink(link) {
            ...
            if (typeof link === "string") {
                ...
                return `<a class="btn btn-primary px-5" href="${link}" target="${defaultTarget}" rel="${defaultRel}">${defaultText}</a>`;
            }
            ...
            if (typeof link === "object" && link.href) {
                ...
                return `<a class="btn btn-primary px-5" href="${link.href}" target="${target}" rel="${rel}">${content}</a>`;
            }
        ...
        function buildFramework($wrapper) {
            ...
            if (settings.search) {
                ...
                const btnCloseSearch = $('<button>', {
                    ...
                    html: `<i class="fa fa-xmark mx-2"></i>`,
                    "aria-label": "Close"
                }).appendTo(topSearchNav);
            ...
            appointments.forEach(appointment => {
                appointment.extras.displayDates.forEach(obj => {
                    ...
                    const appointmentElement = $('<small>', {
                        'data-appointment': true,
                        class: 'px-1 w-100 overflow-hidden mb-1 rounded rounded-1',
                        css: {
                            minHeight: '18px',
                        },
                        html: appointmentContent
            ...
            settings.formatter.window(returnData.appointment, returnData.extras).then(html => {
                ...
                if (!modalExists) {
                    ...
                    const modalHtml = [
                        ...
                        `<button type="button" data-bs-dismiss="modal" class="btn"><i class="fa fa-xmark"></i></button>`,
                        `</div>`,
                ...
                const deleteable = false; // appointment.hasOwnProperty('deleteable') ? appointment.deleteable : true;
                const editable = false; // appointment.hasOwnProperty('editable') ? appointment.editable : true;
                if (editable) {
                    if (!$modal.find('[data-edit]').length) {
                        $(`<button type="button" data-edit class="btn"><i class="fa-solid fa-pen-to-square"></i></button>`).prependTo(modalOptions);
                    }
                } else {
                ...
                if (deleteable) {
                    if (!$modal.find('[data-remove]').length) {
                        $(`<button type="button" data-remove data-bs-dismiss="modal" class="btn"><i class="fa fa-trash"></i></button>`).prependTo(modalOptions);
                    }
                } else {
